...
- Альтернативная гипотеза, $H_{1}$ - наше __предположение__, что человек виновен и для доказательства необходимо собрать
  данные, которые опровергнут $H_{0}$.

Алгоритм проверки гипотез:

- Формулируем $H_{0}$ и $H_{1}$
- Так как в большинстве случаев нам надо проверить факт отличия $H_{1}$ от $H_{0}$, то используем __двустороннюю__
  проверку. И так как такая проверка является бинарной (виновен/невиновен), то можно использовать метрики для бинарных
  тестов (`TPR`, `FPR`, `MDE`). Поэтому, перед началом проведения эксперимента стоит сформулировать и зафиксировать
  желаемые параметры предстоящего эксперимента.
- Проводим эксперимент - собираем доказательства, которые могут опровергнуть $H_{0}$.
- Оцениваем собранные доказательства через вопросы: _“Насколько сильно полученные данные противоречат $H_{0}$?_
  _Достаточно ли силы у полученных данных, чтобы опровергнуть $H_{0}$?”_

## P-value

__Интуитивное определение__

- `P-value` - это способ противопоставить собранные данные заданной $H_{0}$, это возможность
  рассчитать силу доказательства опровергающего $H_{0}$. Можно представлять `P-value`, как некую стандартизированную
  шкалу, которая отвязывает нас от единиц измерения в эксперименте (клики/люди/конверсии/и т.д).
- `P-value` можно считать (воспринимать) как __потенциальный__ `FPR`

Рассмотрим пример с проверкой монеты на честность:

- $H_{0}$ - Орел выпадает в 50% случаев
- $H_{1}$ - Орел не выпадает в 50% случаев
- Пример. Норма $\frac{5}{10}$. Получили $\frac{7}{10}$. Отклонение от нормы (порог) $2$. _Если бы мы отвергали $H_{0}$_
  _с порогом $2$, то какой бы `FPR` мы могли получить? Какой % ложных срабатываний?_ - это и есть `P-value`

<p align="center">
  <img src="source/img/007.png" width="600">
</p>

Есть два способа посчитать `P-value`:

- Через Монте-Карло, проводя виртуальные эксперименты
- Через математическое моделирование (формулы) при условии, что данные соответствуют требованиям к моделям (формулам)

## Проверка применимости СтатТестов к данным

Выше было упомянуто, что посчитать `P-value` можно через математическое моделирование, при условии, что данные будут
соответствовать требованиям используемой модели. И, кажется, что соответствие данным можно проверить эмпирически.

Возьмем все тот же эксперимент с проверкой монетки на честность (Орел в 50% случаев) и:

- Смоделируем `P-value` через Монте-Карло, где `sample_size=10`, `baseline=0.5`, `n=10000`
- Посчитаем `P-value` через Z-test реализованный в библиотеке Statsmodels, где `sample_size=10`, `baseline=0.5`
- Получившуюся разницу можно трактовать следующим образом - _"Если мы будем использовать Z-test для данного случая, то
  математически рассчитанные значения `P-value` будут значительно отличаться (в меньшую сторону) от ожидаемых."_

<p align="center">
  <img src="source/img/008.png" width="450">
</p>

А теперь, в том же эксперименте, посмотрим на разницу ожидаемого/получаемого `FPR` в зависимости от `sample_size`.
Остальные параметры не меняются `baseline=0.5`, `significance=0.05`, `n=10_000`.

<p align="center">
  <img src="source/img/009.png" width="450">
</p>

Видим, что только при sample_size=20 и sample_size=30 мы получаем `FPR` <= `significance` 